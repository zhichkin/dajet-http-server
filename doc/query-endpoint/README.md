...
> **Внимание!** Все параметры тела запроса обязательны! Если входные параметры **parameters** для скрипта отсутствуют, то нужно указывать пустой объект: ```{}```.

Параметр **script** тела запроса должен использовать только команды ```SELECT```, в противном случае будет выдаваться ошибка вида:

```
Any statements, except SELECT, are not supported
```

Любые команды DML (модификация данных) запрещены. Выдаётся ошибка вида:
